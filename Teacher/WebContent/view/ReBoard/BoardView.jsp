...
<script>
	$(document).ready(function() {
		$(".recommen").click(function() {
			...
			//	지금 눌려진 단추가 숨겨놓은 	data-1 이라는 값과
			//										data-2 라는 값을 알아낸다.
			var	reNo = $(this).attr("data-1");
			var	kind = $(this).attr("data-2");
			
			$(location).attr("href", "../ReBoard/ReCommen.sun?reNo="+reNo+"&oriNo="+${VIEW.no}+"&kind=" + kind);
		...
		$("#mBtn").click(function(){
			...
			$(location).attr("href", "../ReBoard/ModifyForm.sun?oriNo=${VIEW.no}&nowPage=${NOWPAGE}");
...
<%--
 ...
 --%>	
		<table width="700" border="1" align="center" id="${data.no}">
			<tr>
				<td>글번호</td>
				<td>${data.no}</td>
				<td>작성날짜</td>
